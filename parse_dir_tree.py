...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 21 21:30:12 2017
...
def main(path='./'):
    ...
    print "Tree listed in: %s" % (filename)
    return tree.write2file(filename)

if __name__ == '__main__':
    print "#-----------------------------------------------------------------"
    print "#- OP4 Utility: "
    print "#-     %s -- %s" % (os.path.basename(sys.argv[0]), 'v1.0')
    print "#-     Usage: %s %s" % (os.path.basename(sys.argv[0]), '"directory path" (defalut: current directory)')
    print "#-     Support: marshals@alchip.com"
    print "#-----------------------------------------------------------------"
    print
    print "Start %s %s..." % (os.path.basename(sys.argv[0]), ' '.join(sys.argv[1:]))
    print ""
    try:
